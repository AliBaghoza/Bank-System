...
public:
    ...
    static   clsCurrency FindByCodeOrCountry(string CodeOrCountry)
    {

        CodeOrCountry = clsString::UpperAllString(CodeOrCountry);
        ...
        if (MyFile.is_open())
        {
            ...
            {
                clsCurrency Currency = _ConvertLinetoCurrencyObject(Line);
                if (Currency.CurrencyCode() == CodeOrCountry || clsString::UpperAllString(Currency.Country()) == CodeOrCountry)
                {
                    MyFile.close();
                    return Currency;
                }
    ...
    {
        clsCurrency C1 = clsCurrency::FindByCodeOrCountry(CurrencyCode);
        return (!C1.IsEmpty());
